...
// Mock data - for owner view we're showing only rooms that belong to the logged-in owner
const ownerRoomsData: Room[] = [
  {
    id: '1',
    number: '101',
...
const OwnerAvailability = () => {
  ...
  const [filteredRooms, setFilteredRooms] = useState<Room[]>(ownerRoomsData);
  
  const calendarDates = generateDates(viewStartDate, displayDays);

  // Apply filters when any filter changes
  useEffect(() => {
    let result = ownerRoomsData;
  ...
  const handleCellClick = (roomId: string, date: Date) => {
    // In a real app, this would open a booking creation form or show details
    toast({
      title: "Room Information",
      description: `Room ${filteredRooms.find(r => r.id === roomId)?.number} on ${format(date, 'MMMM d, yyyy')}`,
    });
  };
    ...
    <div className="animate-fade-in">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">View room availability for your properties</p>
        </div>
        {/* Owner version doesn't include the add booking button */}
      </div>
      ...
      <Card className="mb-8">
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            View room availability for your properties. Click on any day to see more details.
          </CardDescription>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Bookings for your properties in the next 7 days</CardDescription>
        </CardHeader>
        <CardContent>
